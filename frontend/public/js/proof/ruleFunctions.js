...
export class InferenceRulesHelper {
    ...
    proofView.selectAll(".rule").each((x) => {
      ...
      proofView.select("#N" + x.data.source.id).on("click", () => {
        ...
        if (x.data.source.id !== lastToolTipTriggerID) {
          ...
          app.ruleExplanationPosition === "mousePosition"
            ...
            : tooltip.classed(
              this.getPositionClass(app.ruleExplanationPosition),
              true
            );
      ...
      .on("dblclick", (d) => {
        ...
        d.children.forEach(child => {
          children.push(child.data.source.element);
        });
        socket.emit("inference view", { id: d.id, type: d.data.source.element, parent: d.parent.data.source.element, children: children });
      });
    });
  }
  ...
  classHierarchy(premise, tooltip) {
    ...
    if (
      premise[0].split(subsumes)[1].trim() ===
      premise[1].split(subsumes)[0].trim()
    ) {
    ...
    displayObject = tooltip
      .append("span")
      .attr("class", "tooltiptext card-panel")
    ...
    this.addSeparator(
      displayObject,
      this.getLarger(premiseLengths, CLASS_HIERARCHY.length)
    );
    ...
    displayObject
      ...
      .text(leftAxiomLHS + subsumesDisplay)
      .append("span")
    ...
    displayObject
      ...
      .append("span")
      .attr("class", "text-black")
      .text(subsumesDisplay + rightAxiomRHS);
    ...
    displayObject
      .append("span")
      .attr("class", "text-black")
      .text(leftAxiomLHS + subsumesDisplay + rightAxiomRHS);
  }
  ...
  assertedConclusion(conclusion, tooltip) {
    ...
    this.addSeparator(
      displayObject,
      this.getLarger(conclusionLength, ASSERTED_CONCLUSION.length)
    );

    //Add the current instantiation of the rule
    //Add empty premise
    displayObject.append("p").style("margin-bottom", ".8cm");

    //Add the line
    this.addMidRule(displayObject, conclusionLength);

    //Add the conclusion
    displayObject.append("span").attr("class", "text-black").text(conclusion);
  }
  ...
  propertyDomainTranslation(premise, tooltip) {
    ...
    roleName = premise[0]
      .substring(premise[0].indexOf("(") + 1, premise[0].indexOf(")"))
      .trim();
    ...
    this.addSeparator(
      displayObject,
      this.getLarger(premiseLength, PROPERTY_DOMAIN_TRANSLATION.length)
    );
  ...
  intersectionDecomposition(conclusion, tooltip) {
    ...
    lHS = conclusion
      .substring(1, conclusion.indexOf(")"))
      .split(subsumes)[0]
      .trim();
    ...
    this.addSeparator(
      displayObject,
      this.getLarger(conclusionLength, INTERSECTION_DECOMPOSITION.length)
    );
    ...
    //Add the conclusion
    let span = displayObject.append("span"),
      color;
  ...
  addIntersectionDecompositionAbstract(indexOfMatch, displayObject) {
    ...
    let lHSConjuncts = [c1, c2],
      ...
      span = displayObject.append("span"),
      color;
  ...
  equivalentClassesDecomposition(premise, conclusion, tooltip) {
    ...
    this.addSeparator(
      displayObject,
      this.getLarger(premiseLength, EQUIVALENT_CLASSES_DECOMPOSITION.length)
    );
  ...
  existentialPropertyExpansion(premise, tooltip) {
    ...
    this.addSeparator(
      displayObject,
      this.getLarger(premiseLength, EXISTENTIAL_PROPERTY_EXPANSION.length)
    );

    //Add the current instantiation of the rule
    //Add the premise
    displayObject.append("span").attr("class", "text-black").text(premise[0]);
  ...
  intersectionComposition(premise, tooltip) {
    ...
    // let premiseLengths = [premise[0].length, premise[1].length];
    let premiseLengths = [
      lHS.length + rHSConjunct1.length + rHSConjunct2.length,
    ];
    this.addSeparator(
      displayObject,
      this.getLarger(premiseLengths, INTERSECTION_COMPOSITION.length)
    );
  ...
  existentialFillerExpansion(premise, conclusion, tooltip) {
    ...
    this.addSeparator(
      displayObject,
      this.getLarger(conclusionLength, EXISTENTIAL_FILLER_EXPANSION.length)
    );
  ...
  disjointClassesTranslation(premise, conclusion, tooltip) {
    let premiseRoleNames = premise[0]
      .match(regBetweenPar)[0]
      .replace(regPar, "")
      .split(",")
      .map((x) => x.trim());
    let conclusionRoleNames = conclusion
      .match(regBetweenPar)[0]
      .replace(regPar, "")
      .split(and)
      .map((x) => x.trim());
    ...
    this.addSeparator(
      displayObject,
      this.getLarger(premiseLengths, DISJOINT_CLASSES_TRANSLATION.length)
    );
    ...
    for (let i = 0; i < premiseRoleNames.length; i++) {
      let color = conclusionRoleNames.includes(premiseRoleNames[i])
        ? "text-green"
        : "text-black";
  ...
  topSuperClass(conclusion, tooltip) {
    ...
    displayObject
      .append("span")
      .attr("class", "text-black")
      .text(conclusion.split(subsumes)[0] + subsumesDisplay)
      .append("span")
      .attr("class", "text-green")
      .text(conclusion.split(subsumes)[1]);
  }
  ...
  addMidRule(displayObject, length) {
    displayObject
      .append("hr")
      .attr("class", "mid")
      .attr("width", this.getRuleLength(length));
  }
  ...
  getLarger(premiseSizes, ruleNameLength) {
    ...
    return premiseTotalLength > ruleNameLength
      ? premiseSizes
      : [ruleNameLength];
  }

  getPositionClass(ruleExplanationPosition) {
    if (ruleExplanationPosition === "rightBottom") return "positionRB";
    else if (ruleExplanationPosition === "rightTop") return "positionRT";
    else if (ruleExplanationPosition === "leftTop") return "positionLT";
    return "positionLB";
  }
  ...
  setPositionRelativeToMouse(tooltip) {
    ...
    if (element) {
      ...
      let x =
        d3.event.clientX + width > app.contentWidth
          ? app.contentWidth - width
          : d3.event.pageX;
      let y =
        d3.event.clientY + height > app.contentHeight
